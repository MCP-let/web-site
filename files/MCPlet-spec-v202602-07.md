# MCPlet Specification v202602-07
...
MCPlet v202602-07 adopts a **Code-First** model:
...
Each MCPlet tool contract MUST declare one type:
...
## 6. Tool Metadata Contract (`_meta.ui`)

The following fields are defined for MCPlet profile:
...
- `auth` MUST be placed inside `_meta` (sibling of `ui`), NOT at the top level of tool config
...
- Tools with `auth.required=passkey` SHOULD typically be `['app']` (app-only) to enforce authentication before model can invoke
- If exposing authenticated action tools to LLM (visibility `['model']` or `['model','app']`), app or host MUST implement request interception to enforce authentication before forwarding to model
- Read-only queries (visibility `['model']`) SHOULD NOT require Passkey auth
...
MCPlet v202602-07 supports exactly three visibility configurations:
...
- write/sensitive operations SHOULD be app-only,
- read-only queries SHOULD be model-visible,
...
- LLM calls SearchApp tool to get all French restaurants data, becasue SearchApp only has the ability to search for restaurants, not to filter by location
- LLM search restaurants in Shibuya from SearchApp tool's response
...
v202602-07 defines single-layer derived injection only. Multi-hop derived chains are not standardized.
...
- no hidden side effects beyond declared tool type,
- CSP policy for UI resources.

### 16.2 Recommended Controls

- rate limiting for local injection pathways,
- audit logging for app-only sensitive tool calls,
...
### 17.1 A host/server pair claiming MCPlet v202602-07 conformance MUST satisfy:

1. visibility filtering and enforcement,
2. code-first metadata precedence,
3. display mode resolution rules,
4. MCP Apps lifecycle compatibility,
5. stateless MCPlet execution model.
...
## Appendix C: Reference Implementation Mapping (Informative)

This version is informed by:

- `reference_impl_restaurant_reservations/mcpapps/mcp-server/index.ts` - Tool server (verified stable with v202602-07)
...
**End of MCPlet Specification v202602-07**
